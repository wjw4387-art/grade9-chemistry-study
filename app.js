...
function activate(data){
  ...
  state=cleanProgress(record.courses[course.id],new Set(lessonById.keys()),new Set(questionById.keys()),questionById);
...
function route(){const parts=(location.hash||'#/courses').slice(1).split('/').filter(Boolean);return {name:parts[0]||'home',id:parts[1]};}
...
function portalFrame(page){
  ...
  return `<a class="skip-link" href="#main" data-action="skip-content">跳转到正文</a><header class="portal-header"><a class="brand" href="#/courses"><span class="brand-icon">知</span><span class="brand-copy">知学<span class="brand-sub">自主学习空间</span></span></a>${recent?`<a class="text-link" href="#/course/${recent.id}">继续学习 · ${recent.title} →</a>`:'<span class="muted-label">每一次好奇，都值得探索</span>'}</header><main id="main" class="portal-main" tabindex="-1">${page}<footer class="footer"><span>知学 · 把知识学明白</span><span>自由选课 · 按自己的节奏学习</span></footer></main><div id="dialog-root"></div>`;
}
...
function homePage(){
  ...
  <div class="hero-stats"><div class="stat-block"><span>课程内容</span><strong>${allLessons.length}<small> 小课</small></strong><p>${course.shortScope} · ${units.length} ${course.unitLabel}</p></div><div class="stat-block"><span>已经掌握</span><strong>${done}<small> / ${allLessons.length}</small></strong><p>按你的节奏积累</p></div><div class="stat-block"><span>分层题库</span><strong>${questionById.size}<small> 题</small></strong><p>基础 → 提高 → 中考挑战</p></div><a class="stat-block" href="#/review"><span>待复习错题</span><strong>${Object.keys(state.wrong).length}<small> 题</small></strong><p>复习巩固 ↗</p></a></div>
...
function lessonPage(id){
  ...
  const diagram=dataset.diagramFor(id);
...
function questionFigure(q,zoom=false){
  ...
  const figure=svg?`<figure class="diagram-card question-diagram">${svg}<figcaption>题目示意图 · 请结合题干中的条件作答</figcaption></figure>`:'';
...
function questionCard(q,index){
  ...
  return `<section class="panel question"><div class="reading-meta"><span class="badge difficulty-${q.difficulty}">${{1:'基础',2:'提高',3:'中考挑战'}[q.difficulty]}</span><span class="meta">${{choice:'单选题',multi:'多选题',text:'填空题',case:'综合探究'}[q.type]}</span><span class="muted-label">${q.type==='multi'?'选出所有正确选项':q.type==='case'?'每一小问都需要作答':''}</span></div><h2>${esc(q.stem)}</h2>${questionFigure(q)}${options}${item?`<div class="feedback ${item.correct?'':'bad'}"><strong>${item.correct?'✓ 回答正确':'再理解一下这道题'}</strong><p>参考答案：${esc(expectedAnswer(q))}</p><p>${esc(q.explain)}</p><a class="text-link" href="#/lesson/${q.lessonId}">回看：${lessonById.get(q.lessonId).title} ↗</a></div>`:''}</section>`;
}
function testRun(){
...
async function importProgress(file){
  ...
  try{const data=JSON.parse(await file.text());if(data.format!=='zhixue-progress'||data.courseId!==course.id||data.version!==2||!data.progress)throw new Error();const imported=cleanProgress(data.progress,new Set(lessonById.keys()),new Set(questionById.keys()),questionById);showDialog('导入这份学习记录？',`文件含 ${Object.keys(imported.statuses).length} 项学习状态、${Object.keys(imported.wrong).length} 道错题。导入后将替换当前${course.title}记录。`,'确认导入',()=>{state=imported;activeTest=state.draft?{...state.draft,submitted:false}:null;result=null;save();render();toast('学习记录已导入。');});}catch{toast('无法识别此文件，请选择本网站导出的学习记录。');}
}
...
document.addEventListener('click',event=>{
  ...
  if(action==='zoom'){const lesson=lessonById.get(id);showDialog(lesson.title+' · 知识图示','','',null,dataset.diagramFor(id));}
