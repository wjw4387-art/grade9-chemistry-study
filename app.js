import {questionBadge,answerInstructions,correctAnswerHTML,chemistryReference} from './question-presentation.js?v=20260926-sixteen-type1';
import {explanationHTML} from './question-explanations.js?v=20260926-sixteen-type1';
import {applyReadingSize,readingControl,readableDiagram} from './reading-settings.js?v=20260926-sixteen-type1';
import {formatChemistryText} from './chemistry/equations.js?v=20260926-sixteen-type1';
import {courses, stages, courseMeta, courseIdForRoute, loadCourse} from './course-registry.js?v=20260926-sixteen-type1';
import {grade, isAnswered, drawByDifficulty, cleanProgress} from './learning-core.js?v=20260926-sixteen-type1';
...
function frame(page,current,selected){
  ...
  <aside class="app-sidebar" aria-label="学习导航"><button class="sidebar-close" data-action="menu" aria-label="关闭学习导航">✕</button><a class="brand" href="#/courses"><span class="brand-icon">学</span><span class="brand-copy">学之路<span class="brand-sub">自主学习空间</span></span></a>
  ...
  <main id="main" class="main" tabindex="-1">${storageOK?'':'<div class="inline-note">浏览器未能保存记录。请先导出备份，并检查浏览器存储设置。</div>'}${page}<footer class="footer"><span>学之路 · 把知识学明白</span><span>${course.edition} · ${course.title} · 记录保存在当前浏览器</span></footer></main></div></div><div id="dialog-root"></div>`;
}
...
function portalFrame(page){
  ...
  return `<a class="skip-link" href="#main" data-action="skip-content">跳转到正文</a><header class="portal-header"><a class="brand" href="#/courses"><span class="brand-icon">学</span><span class="brand-copy">学之路<span class="brand-sub">自主学习空间</span></span></a>${readingControl()}${recent?`<a class="text-link" href="#/course/${recent.id}">继续学习 · ${recent.title} →</a>`:'<span class="muted-label">每一次好奇，都值得探索</span>'}</header><main id="main" class="portal-main" tabindex="-1">${page}<footer class="footer"><span>学之路 · 把知识学明白</span><span>自由选课 · 按自己的节奏学习</span></footer></main><div id="dialog-root"></div>`;
}
...
function homePage(){
  ...
  <div class="hero-stats"><div class="stat-block"><span>课程内容</span><strong>${allLessons.length}<small> 小课</small></strong><p>${course.shortScope} · ${units.length} ${course.unitLabel}</p></div><div class="stat-block"><span>已经掌握</span><strong>${done}<small> / ${allLessons.length}</small></strong><p>按你的节奏积累</p></div><div class="stat-block"><span>分层题库</span><strong>${questionById.size}<small> 题</small></strong><p>每课16题 · 四级递进</p></div><a class="stat-block" href="#/review"><span>待复习错题</span><strong>${Object.keys(state.wrong).length}<small> 题</small></strong><p>复习巩固 ↗</p></a></div>
...
function unitPage(id){
  ...
  return `<div class="crumb"><a href="#/home">学习地图</a><span>/</span><span>${termName(unit.term)} · 第${unit.number}${course.unitLabel}</span></div>${pageHeading(`${termName(unit.term)} · ${course.unitLabel} ${String(unit.number).padStart(2,'0')}`,unit.title,unit.intro)}<div class="toolbar"><button class="btn primary" data-action="test-unit" data-id="${unit.id}">本${course.unitLabel}测试 · ${course.subject==='chemistry'?16:24}题</button><span class="chip done">${percent(unit)}% 已掌握</span><span class="muted-label">先选课，或先做题了解自己的掌握程度</span></div><div class="filterbar" aria-label="学习状态筛选">${[['all','全部'],...Object.entries(labels)].map(([id,label])=>`<button class="btn small ${unitFilter===id?'active':''}" data-action="filter" data-id="${id}" aria-pressed="${unitFilter===id}">${label}</button>`).join('')}</div><div class="lesson-list">${lessons.length?lessons.map(lessonRow).join(''):'<div class="empty-state">这个状态下还没有小课。切换“全部”可查看本章内容。</div>'}</div>`;
}
...
function lessonPage(id){
  ...
  <div class="lesson-bottom"><div><h2>懂了，就用题目检验一下。</h2><p>${l.questions.length} 道题，每4题提升一级，最后4题为拓展拔高。</p></div><button class="btn primary" data-action="test-lesson" data-id="${id}">开始本课练习 →</button></div><div class="toolbar"><button class="btn" data-action="status" data-id="${id}" data-value="done">✓ 标记已掌握</button><button class="btn ghost" data-action="status" data-id="${id}" data-value="skipped">暂时跳过</button>${next?`<a class="next-lesson" href="#/lesson/${next.id}">下一课：${next.title} →</a>`:''}</div></article>
...
function archiveNote(){const a=state.archive;if(!a)return '';return `<div class="inline-note">题库已更新为每课16题、四档难度。${Object.keys(a.wrong||{}).length}道旧版错题${a.draft?'和一份旧版练习草稿':''}已归档，仍包含在导出的学习记录中；新练习使用修订题库。学习状态与历史成绩保留。<a href="#/about">导出记录 ↗</a></div>`;}
function reviewPage(){
...
function makeTest(type,id){
  ...
  const limit=['lesson','question'].includes(type)?pool.length:course.subject==='chemistry'?16:type==='wrong'?pool.length:type==='unit'?24:40;
...
function questionCard(q,index){
  ...
  return `<section class="panel question"><div class="reading-meta"><span class="badge difficulty-${q.difficulty}">${(course.difficultyLabels||{1:'基础理解',2:'应用辨析',3:'中考综合',4:'拓展拔高'})[q.difficulty]}</span>${questionBadge(q)}${q.kind?`<span class="meta">${esc(q.kind)}</span>`:''}</div><p class="question-stage-note">第 ${q.difficulty} 级 / 4${q.difficulty===4?' · 拓展拔高：可以先做前面的题，再回来挑战。':''}</p>${answerInstructions(q)}<h2>${study(q.stem)}</h2>${course.subject==='chemistry'?`<details class="question-reference"><summary>查看计算参考数据</summary><p>${esc(chemistryReference)}</p></details>`:''}${questionFigure(q)}${options}${item?`<div class="feedback ${item.correct?'':'bad'}">${correctAnswerHTML(q,study)}<p class="answer-verdict ${item.correct?'correct-text':'incorrect-text'}">${item.correct?'✓ 回答正确':'本题尚未答对，请对照答案查看解析。'}</p>${explanationHTML(q,lessonById.get(q.lessonId),details[q.lessonId],study)}</div>`:''}</section>`;
}
function testRun(){
  if(!activeTest)return testHome();
  const t=activeTest,q=questionById.get(t.ids[t.index]),count=answeredCount();
  return `<div class="crumb"><a href="#/test">练习与测试</a><span>/</span><span>${esc(t.title)}</span></div>${pageHeading(t.submitted?'REVIEW YOUR ANSWERS':'ONE STEP AT A TIME',esc(t.title),t.submitted?'逐题查看答案与解析，找到需要巩固的地方。':'专注这一题。作答自动保存，可随时切换题目或稍后继续。')}${result?`<section class="activity-card result-card"><div><span class="header-kicker">本次完成</span><h2>答对 ${result.correct} <small>/ ${result.total} 题</small></h2><p>${result.correct===result.total?'全部答对，做得很扎实。':'答错和未作答的题已加入复习本。综合题全部小问正确计为答对。'}</p></div><div class="toolbar">${t.type==='lesson'?`<button class="btn primary" data-action="status" data-value="done" data-id="${t.id}">标记本课已掌握</button>`:''}<a class="btn" href="#/review">查看复习本</a></div></section>`:''}<div class="quiz-layout"><div><div class="quiz-progress"><strong>第 ${t.index+1} 题 <span>/ ${t.ids.length}</span></strong><span>${t.submitted?'解析模式':`已答 ${count} 题`}</span></div>${questionCard(q,t.index)}<div class="quiz-actions"><button class="btn" data-action="quiz-prev" ${t.index===0?'disabled':''}>← 上一题</button><button class="btn primary" data-action="${t.index<t.ids.length-1?'quiz-next':t.submitted?'test-home':'submit'}">${t.index<t.ids.length-1?'下一题 →':t.submitted?'返回练习中心':'提交并查看解析'}</button></div></div><aside class="quiz-aside panel"><h3>答题卡</h3>${t.type==='lesson'?'<p class="quiz-tier-summary">1—4 基础理解<br>5—8 应用辨析<br>9—12 中考综合<br>13—16 拓展拔高</p>':'<p class="quiz-tier-summary">按四档难度，由易到难排列</p>'}<p class="muted-label">点击题号，自由切换</p><div class="quiz-nav">${t.ids.map((id,i)=>{const item=result?.items[i];return `<button class="quiz-dot ${i===t.index?'current':''} ${item?(item.correct?'correct':'incorrect'):isAnswered(questionById.get(id),t.answers[id])?'answered':''}" data-action="quiz-goto" data-id="${i}" aria-label="第${i+1}题${questionById.get(id).type==='multi'?'，多选题':''}${item?(item.correct?'，正确':'，有误'):isAnswered(questionById.get(id),t.answers[id])?'，已答':'，未答'}" ${i===t.index?'aria-current="step"':''}><span>${i+1}</span>${questionById.get(id).type==='multi'?'<span class="quiz-multi-marker">多选</span>':''}</button>`;}).join('')}</div><div class="quiz-legend"><span>○ ${t.submitted?'绿：正确':'空白：未答'}</span><span>● ${t.submitted?'橙：有误':'绿色：已答'}</span></div>${!t.submitted?`<button class="btn primary full-width" data-action="submit">提交练习</button><p class="muted-label save-note">作答保存在当前浏览器</p>`:'<a class="btn full-width" href="#/test">选择其他测试</a>'}</aside></div>`;
}
function submitQuiz(){
...
function aboutPage(){
  ...
  return `${pageHeading('ABOUT THIS COURSE','课程说明与学习记录','清楚内容从哪里来，也保管好自己的学习进度。')}<section class="panel teaching-section"><h2>${course.title} · 教材范围</h2>${physics?`<p>按人民教育出版社八年级上册、八年级下册、九年级全一册的新教材组织，覆盖22章，包括实验探究与新版跨学科实践。每一节均可独立学习和练习。</p><p>版本核验日期：2026年9月26日。对应人教版2024修订教材体系，并结合2025日常修订版课程标准信息；目前未找到另有一套“2026独立修订版”教材的出版证据。网页中的“截至2026核验”指资料核验时间。</p><p>新版包含二力合成、抽水机、杆秤等内容；九年级热学顺序、家庭电路、通信和能源章节也按新目录编排。学校采用其他出版社时，知识大体相通，但课序可能不同。</p><p>核心定义按课程概念归纳，讲解、例题、图示和题目原创。项目目前没有物理原始试卷，题目按中考常见实验、图像、计算与推理形式编写，不冒称来自所提供的化学卷。</p><p><a class="text-link" href="https://www.pep.com.cn/xw/zt/hd/12/xjcjs/cz/202409/t20240925_1995627.html" target="_blank" rel="noopener">人教社新物理教材说明 ↗</a> · <a class="text-link" href="https://xuanshu.hep.com.cn/front/h5Mobile/bookDetails?bookId=697a4254e119ac972922bf54" target="_blank" rel="noopener">2025修订课标官方解读信息 ↗</a></p>`:`<p>按人民教育出版社九年级化学新教材整理：上册第1—7单元，下册第8—11单元。覆盖绪言、课题、实验活动和跨学科实践，部分长课题拆分为自主小课。</p><p>化学为人教版新教材，上册2024启用、下册2025启用。没有把出版年份改称“2026新版”。核心定义按教材含义归纳，讲解、图示和习题原创；上册题型参考项目提供的练习册与解析资料，下册沿用同样分层标准。化学练习现为每课16题、每4题一级，依次为基础理解、应用辨析、中考综合、拓展拔高；最后2题继续按高于广州中考综合题的迁移要求设计；难度为教学设计分级，尚无学生作答样本校准。</p>`}${physics?'':`<p>参考本地练习册和广州中考的证据推理、陌生信息迁移、工艺流程与实验探究要求，题目均为原创，未冒称广州中考原题。<a class="text-link" href="https://jyj.gz.gov.cn/yw/zsks/content/post_10341699.html" target="_blank" rel="noopener">广州2025化学命题说明 ↗</a> · <a class="text-link" href="https://jyj.gz.gov.cn/gkmlpt/content/9/9738/post_9738644.html" target="_blank" rel="noopener">广州2024命题说明 ↗</a></p>`}<p>填空和综合题按规范答案集合匹配，不能理解所有同义长句；请按题目指定的单位和格式填写。公式中的大小写、单位和数量关系均有明确含义。实验学习应遵循教材操作规范，在教师指导下进行。</p><p><a class="text-link" href="https://jc.pep.com.cn/" target="_blank" rel="noopener">人教社电子教材 ↗</a></p></section><section class="panel"><h2>学习记录备份</h2>${archiveNote()}<p>当前课程：${course.title}。进度、错题和未完成练习保存在当前浏览器，换设备或清理浏览器数据后不会自动同步。可以导出备份，在另一台设备导入继续学习。</p><div class="toolbar"><button class="btn primary" data-action="export">导出本课程记录</button><label class="btn" for="import-progress">导入本课程记录</label><input class="visually-hidden" id="import-progress" type="file" accept="application/json,.json"></div><p class="muted-label">导入前会检查课程是否一致，确认后仅替换本课程的学习记录。</p></section>`;
}
...
async function renderPage(options={}){
  ...
  if(r.name==='courses'&&!search.trim()){
    save();document.getElementById('app').innerHTML=portalFrame(coursesPage());document.title='课程中心 · 学之路';
  ...
  if(!course||course.id!==desired){
    save();
    document.getElementById('app').innerHTML=`<main class="loading-state" aria-busy="true"><span class="brand-icon">学</span><h1>正在打开${esc(courseMeta(desired)?.title||'课程')}…</h1><p>载入讲解、图示与练习题</p></main>`;
  ...
  document.title=`${r.name==='courses'?'课程中心':r.name==='lesson'?(lessonById.get(r.id)?.title||course.title):course.title} · 学之路`;
}
...
function captureAnswer(input){
  ...
  const dot=document.querySelector(`.quiz-dot[data-id="${activeTest.index}"]`);dot?.classList.toggle('answered',isAnswered(q,activeTest.answers[id]));dot?.setAttribute('aria-label',`第${activeTest.index+1}题${q.type==='multi'?'，多选题':''}，${isAnswered(q,activeTest.answers[id])?'已答':'未答'}`);
